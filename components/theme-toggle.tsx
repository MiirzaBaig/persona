...
interface ThemeToggleProps {
  /** Use when the toggle is on a dark background (e.g. footer) for correct contrast */
  onDarkBackground?: boolean;
}

export default function ThemeToggle({ onDarkBackground = false }: ThemeToggleProps) {
  const ColorTheme = {
    Light: "light",
    Dark: "dark",
    System: "system",
  ...
  const trackClass = onDarkBackground
    ? "bg-whiteout/10 border border-whiteout/20 rounded-full"
    : "border-2 border-black dark:border-zinc-100 bg-whiteout dark:bg-zinc-900 rounded-full shadow-neobrutalism dark:shadow-neobrutalism-dark";

  const knobClass = onDarkBackground
    ? "w-[26px] h-[26px] bg-whiteout rounded-full shadow-sm"
    : "w-[26px] h-[26px] bg-blackout dark:bg-whiteout rounded-full border border-black dark:border-zinc-100";

  const iconActiveClass = onDarkBackground ? "fill-blackout" : "fill-blackout dark:fill-whiteout";
  const iconInactiveClass = onDarkBackground ? "fill-whiteout/60" : "fill-blackout/50 dark:fill-zinc-100/50";

  return (
    <div className="w-full flex justify-center items-center md:w-auto md:block" suppressHydrationWarning>
      <div className="relative">
        <motion.div
          className={`absolute w-28 h-9 top-0 left-0 flex items-center px-2 rounded-full ${trackClass}`}
          style={{
            justifyContent:
              theme === "system" ? "center" : theme === "light" ? "start" : theme === "dark" ? "end" : undefined,
          }}
        >
          <motion.div layout transition={{ type: "spring", stiffness: 400, damping: 30 }} className={`flex-shrink-0 ${knobClass}`} />
        </motion.div>
        <div className="h-9 w-28 flex justify-between items-center relative px-1">
          <button
            type="button"
            className="h-full w-1/3 flex items-center justify-center rounded focus:outline-none focus-visible:ring-2 focus-visible:ring-whiteout/50 focus-visible:ring-offset-2 focus-visible:ring-offset-blackout"
            onClick={() => setTheme("light")}
            title="Light"
            aria-pressed={theme === "light"}
          >
            <svg width="16" height="16" viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg" className={theme === ColorTheme.Light ? iconActiveClass : iconInactiveClass} aria-hidden>
          ...
          >
            <svg width="16" height="16" viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg" className={theme === "system" ? iconActiveClass : iconInactiveClass} aria-hidden>
          ...
          >
            <svg width="16" height="16" viewBox="0 0 17 17" fill="none" xmlns="http://www.w3.org/2000/svg" className={theme === ColorTheme.Dark ? iconActiveClass : iconInactiveClass} aria-hidden>
